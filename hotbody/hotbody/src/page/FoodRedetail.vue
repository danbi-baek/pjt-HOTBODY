<template>
  <div class="detailpage">
    ...
    <div class="detailfood">
        <div class="foodinfo">
            <div class="foodimg">
                <img style="width:110%;height:110%" :src="foodTotalImg"/>
            </div>
            <div class="foin">
                <div class="eatinfo">
                ...
                <div class="foodinfos">
                    <div class="foinfo" v-if="getFood.length!=0">
                        <div class="foods" v-for="(food, i) in getFood.imgs " :key="i"  >
                            <div class="foodinfoimg" @click="writeDetail(i)">
                                <img class="imgh" :src="'http://127.0.0.1:8000'+getFood.imgs[i].foodImg"/>
                            </div> 
                    ...
                    <!-- <div class="foinfo" v-if="getFood.length==0">
                        <div>
                            <div class="foods">
                                <div  @click="show"><i class="isize fas fa-plus" ></i><br>음식 추가하기</div>
                            </div>
                        </div>
                    </div> -->
                </div>
            ...
            <div class="foodamount" v-if="this.tab!=0">
                ...
                <div class="amountmenu" v-if="this.tab==1">
                    <div>
                        ...
                        <div class="values">
                            <p>탄수화물  :  {{Math.round(getFood.imgs[index].car/100*detailFood.gram*100)/100.00}} g</p>
                            <p>단백질  :  {{Math.round(getFood.imgs[index].pro/100*detailFood.gram*100)/100.00}} g</p>
                            <p>지방  :  {{Math.round(getFood.imgs[index].fat/100*detailFood.gram*100)/100.00}} g</p>
                            <p>당분  :  {{Math.round(getFood.imgs[index].tsg/100*detailFood.gram*100)/100.00}} g</p>
                            <p>칼로리  :  {{Math.round(getFood.imgs[index].kcal/100*detailFood.gram*100)/100.00}} kcal</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="btns" v-if="!detailOpen">
            <button @click="updatetodayDiet">
                    수정하기
            </button>
            <button @click="delDiet">
                    삭제하기
            </button>
        </div>
    <main-footer/>
...
export default {
    ...
    created () {
        ...
        axios.get('http://127.0.0.1:8000/foods/detail',{
            params: {
                pk : localStorage.getItem('userpk'),
                date : localStorage.getItem('d'),
                type : this.type
            }
            })
            .then( res => {
                ...
                this.$store.state.addFoodList = {totalimg:res.data.uploadimg.eatImg, data:res.data.food, imgs:res.data.img}
                this.$store.state.godetail = false
            })
            .catch( err => {
                err
            })
            .finally( () =>{
                this.getFood = this.$store.state.addFoodList
                this.foodTotalImg = 'http://127.0.0.1:8000' + this.$store.state.addFoodList.totalimg
                // console.log(this.foodTotalImg)
                // console.log(this.getFood.data) // gram 잇는거 
                // console.log(this.getFood.imgs) //gram 무조건 100
    ...
    methods:{
        ...
        updateInput: function(event){
            ...
            this.getFood.data[this.index].gram = updated;
        },
        getNut(){
            this.$store.state.detailNut = [0,0,0,0]
            for(var i=0;i<this.getFood.data.length;i++){
                this.$store.state.detailNut[0] += this.getFood.data[i].kcal
                this.$store.state.detailNut[1] += this.getFood.data[i].car
                this.$store.state.detailNut[2] += this.getFood.data[i].pro
                this.$store.state.detailNut[3] += this.getFood.data[i].fat
            }
            for(i =0 ; i<4;i++){
        ...
        writeDetail(i){
            ...
            this.tab = 1
            this.index = i
            this.detailFood = this.getFood.data[i]
        },
        delFood(i){
            // 음식 삭제
            // console.log('음식삭제````')
            this.detailOpen = false
            this.getFood.imgs.splice(i,1)
            this.getFood.data.splice(i,1)
            this.getNut()
            // console.log('-------');
        },
        foodsearch(){
        ...
        addFood(i){
            ...
            if(this.searchtype == 0 ) { //그냥 추가
                this.getFood.imgs.push(this.searchInfo[i])
                this.getFood.data.push(this.searchInfo[i])
                this.detailOpen = false
            } else if(this.searchtype == 1) { // 수정 추가
                this.getFood.imgs[this.index] = this.searchInfo[i]
                this.getFood.data[this.index] = this.searchInfo[i]
                this.detailOpen = false
            }
            this.getFood.imgs.gram = 100
            this.searchtype = 0
            this.getNut()
            this.hide()
        ...
        updatetodayDiet(){  // --> 삭제 & 재 업로드.....
        if(this.getFood.data.length == 0) {
            alert('식단을 추가 해 주세요!')
        } else {
            // this.delDiet()
            axios.delete(`http://127.0.0.1:8000/foods/delete/`,{
                params: {
                    pk : localStorage.getItem('userpk'),
                    type : this.type,
                ...
                // console.log(this.getFood.data)
            .then(res => {
                res
                axios.post(`http://127.0.0.1:8000/foods/add/`,{
                    params: {
                        pk : localStorage.getItem('userpk'),
                        lis : this.getFood.data,
                        type : this.type,
                        date : this.$store.state.clickdate
                    }
                })
        ...
        delDiet(){
            axios.delete(`http://127.0.0.1:8000/foods/delete2/`,{
                params: {
                    pk : localStorage.getItem('userpk'),
                    type : this.type,
                    date : this.$store.state.clickdate
        ...
        gramModify(){
            var gf = this.getFood.data[this.index]
            var df = this.getFood.imgs[this.index]
